...
public class GameService {
	...
	public GameRoom buildDdzGameRoom(String memberId, List<String> lawNames)
			throws NoServerAvailableForGameException, IllegalGameLawsException {
		List<GameServer> allServers = gameServerDao.findByGame(Game.doudizhu);
		if (allServers == null || allServers.isEmpty()) {
			throw new NoServerAvailableForGameException();
		}
		Random r = new Random();
	 ...
	 * 创建大菠萝房间
	 */
	public GameRoom buildDblGameRoom(String memberId, List<String> lawNames)
			throws NoServerAvailableForGameException, IllegalGameLawsException {
		List<GameServer> allServers = gameServerDao.findByGame(Game.daboluo);
		if (allServers == null || allServers.isEmpty()) {
			throw new NoServerAvailableForGameException();
		}
		Random r = new Random();
	 ...
	 * 创建跑得快房间
	 */
	public GameRoom buildPdkGameRoom(String memberId, List<String> lawNames)
			throws NoServerAvailableForGameException, IllegalGameLawsException {
		List<GameServer> allServers = gameServerDao.findByGame(Game.paodekuai);
		if (allServers == null || allServers.isEmpty()) {
			throw new NoServerAvailableForGameException();
		}
		Random r = new Random();
