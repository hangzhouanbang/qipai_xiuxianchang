...
import com.anbang.qipai.xiuxianchang.msg.channel.sink.PaodekuaiGameSink;
...
public class PaodekuaiGameMsgReceiver {
	...
	public void receive(CommonMO mo) {
		...
		if ("playerQuit".equals(msg)) {// 有人退出游戏
			...
			GameRoom room = gameService.findGameRoomByGame(Game.paodekuai, gameId);
			if (room != null) {
				try {
					gameRoomCmdService.leaveGameRoom(gameId, playerId);
					gameService.leaveGameRoom(Game.paodekuai, gameId, playerId);
				} catch (Exception e) {
					e.printStackTrace();
				}
			}
		...
		if ("ju canceled".equals(msg)) {// 取消游戏
			...
			List<MemberGameRoom> memberRooms = gameService.findMemberGameRoomByGame(Game.paodekuai, gameId);
			...
			gameService.updateGameRoomFinishedByGame(Game.paodekuai, gameId, true);
			gameService.finishMemberGameRoom(Game.paodekuai, gameId);
		}
		...
		if ("game delay".equals(msg)) {// 游戏延时
			Map data = (Map) mo.getData();
			String gameId = (String) data.get("gameId");
			GameRoom gameRoom = gameService.findGameRoomByGame(Game.paodekuai, gameId);
			// 延时1小时
			gameService.delayGameRoom(Game.paodekuai, gameId, gameRoom.getDeadlineTime() + 1 * 60 * 60 * 1000);
		}
	}
}
