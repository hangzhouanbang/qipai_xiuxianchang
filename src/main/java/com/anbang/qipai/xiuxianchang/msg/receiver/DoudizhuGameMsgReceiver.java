...
import com.anbang.qipai.xiuxianchang.msg.channel.sink.DoudizhuGameSink;
...
public class DoudizhuGameMsgReceiver {
	...
	public void receive(CommonMO mo) {
		...
		if ("playerQuit".equals(msg)) {// 有人退出游戏
			...
			if (room != null) {
				try {
					...
					gameService.leaveGameRoom(Game.doudizhu, gameId, playerId);
				} catch (Exception e) {
					e.printStackTrace();
				}
			}
		...
		if ("ju canceled".equals(msg)) {// 取消游戏
			...
			List<MemberGameRoom> memberRooms = gameService.findMemberGameRoomByGame(Game.doudizhu, gameId);
